...
def validate():
    ...
    if len(username) < 3 or len(username) > 21 or username == '' or ' ' in username:
        username_error = "Please enter a valid username" 
        username = ''

    if len(password) < 3 or len(password) > 21 or password == '' or ' ' in password: 
    ...
    if email == '':
        ...
        if len(email) < 3 or len(email) > 21 or '@' not in email or '.' not in email:
